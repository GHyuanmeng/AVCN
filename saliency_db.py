...
def make_dataset(root_path, annotation_path, salmap_path, audio_path,
	...
	dataset = []
	audiodata= []
	gt_root = r'/home/qingda/Data/stavis_dataset/av_txt/'
	data_name = root_path.split('/')[6]
	for i in range(len(video_names)):
		...
		for j in range(1, n_frames, step):
			...
			gt_path = gt_root + data_name + '/' + str(video_names[i]) + '.txt'
			lines = [line.rstrip('\n') for line in open(gt_path)]
			try:

				sample_j['frame_indices_gt'] = list(lines[num - 1][-4:-3] for num in list(sample_j['frame_indices']))
...
class saliency_db(data.Dataset):
	...
	def __getitem__(self, index):
		...
		gt_indices = self.data[index]['frame_indices_gt']
		if self.temporal_transform is not None:
			frame_indices, gt_indices = self.temporal_transform(frame_indices, gt_indices)

		gt_num = gt_value(gt_indices)
		gt_data = int(gt_num)
		...
		return data, valid, target, gt_data
